...
class c_daur_ulang extends CI_Controller {
    ...
    public function index() {
        ...
        $data['daur_ulang'] = $this->m_daur_ulang->get_all_daur_ulang();
    ...
    public function harian() {
        ...
        $user_id = $this->session->userdata('user_id');
        $data['user'] = $this->m_users->get_user_by_id($user_id);
        $data['daur_ulang'] = $this->m_daur_ulang->get_harian($tanggal, $user_id); // Add user_id
        $data['nama_bulan'] = $nama_bulan;
        $data['tahun'] = $tahun;
        $data['kategori'] = $this->m_ktgrsampah->get_all_ktgrsampah($user_id); // Add user_id
        $data['tps'] = $this->m_tps->get_all_tps($user_id); // Add user_id

        $this->load->view('templates/header', $data);
        $this->load->view('templates/topbar', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('v_daur_ulang/create', $data);
        $this->load->view('templates/footer');
    }
    ...
    public function kalender() {
        ...
        $bulan = date('n');
        $tahun = date('Y');
        $data['kalender_data'] = $this->m_daur_ulang->get_data_bulanan($bulan, $tahun);
    
        $tanggal = date('Y-m-d');
        $nama_bulan = $this->get_bulan_indonesia($bulan);
        $data['daur_ulang'] = $this->m_daur_ulang->get_harian($tanggal);
        $data['nama_bulan'] = $nama_bulan;
        $data['tahun'] = $tahun;
        $data['kategori'] = $this->m_ktgrsampah->get_all_ktgrsampah($user_id);
        $data['tps'] = $this->m_tps->get_all_tps($user_id);
    ...
    public function view_details($tps_id) {
        ...
        $this->load->view('templates/topbar', $data);
        $this->load->view('templates/sidebar', $data);
        $this->load->view('v_daur_ulang/view_details', $data); // Create view_details.php for detailed view
        $this->load->view('templates/footer');
    }  
